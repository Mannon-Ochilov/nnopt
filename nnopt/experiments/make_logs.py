...
def latency_blocked(fh):
    ...
    header(fh, "ENCODER LATENCY LIBRARY -- RUN 1, BLOCKED DESIGN",
           ...
           next (blocked by configuration), 3 warm-up and 10 measured runs
           per configuration, median reported, single thread.
           This is the run reported in Table 11 of the article.
           INT8 was measured FIRST in this sequence and is the one row that
           did not reproduce under run 2: 8658.2 ms here against 6981.2 ms
           interleaved, 24% apart, while the other eleven rows agree within
           -0.4% to +6.7%. See logs/latency_library_interleaved.log.
           """)
...
def latency_interleaved(fh):
    ...
    header(fh, "ENCODER LATENCY LIBRARY -- RUN 2, INTERLEAVED DESIGN",
           ...
           run-to-run spread (-0.4% to +6.7%); the blind INT8 row does not
           (8658.2 ms in run 1, 6981.2 ms here, 24% apart). Run 1's value
           lies 17% above the maximum of the seven repeats below, so it is
           outside this distribution rather than an extreme sample of it.
           Because every speedup in Table 11 is computed against that
           baseline, the whole ratio column moves with it: 1.06x-1.14x here
           against 1.29x-1.40x in run 1. This run is the corrected record.
           """)
